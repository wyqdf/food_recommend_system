...
const MODE_CACHE_TTL = 20 * 60 * 1000
const CATEGORY_CACHE_TTL = 60 * 60 * 1000
const CATEGORY_CACHE_KEY = 'home_categories'

const readCachedJSON = (key, ttl) => {
    if (typeof window === 'undefined') return null
    try {
        const raw = sessionStorage.getItem(key)
        const timestamp = Number(sessionStorage.getItem(`${key}__time`))
...
const writeCachedJSON = (key, value) => {
    ...
    sessionStorage.setItem(`${key}__time`, Date.now().toString())
}
...
const loadCategories = async () => {
    const cached = readCachedJSON(CATEGORY_CACHE_KEY, CATEGORY_CACHE_TTL)
    if (cached?.length) {
    ...
    try {
        ...
        writeCachedJSON(CATEGORY_CACHE_KEY, categories.value)
    } catch (error) {
        console.error('分类加载失败:', error)
    }
}

const loadModeSections = async (preferCache = true) => {
    const recommendKey = `home_recommend_${currentMode.value}`
    const hotKey = `home_hot_${currentMode.value}`
    let cachedRecommend = null
    let cachedHot = null

    if (preferCache) {
        cachedRecommend = readCachedJSON(recommendKey, MODE_CACHE_TTL)
        cachedHot = readCachedJSON(hotKey, MODE_CACHE_TTL)
        if (cachedRecommend) {
            recommendList.value = cachedRecommend
        }
        if (cachedHot) {
            hotList.value = cachedHot
        }
        if (cachedRecommend && cachedHot) {
            loading.value = false
            return
        }
    }
    ...
    try {
        const [recRes, hotRes] = await Promise.all([
            ...
            recipeApi.getList({ sort: 'hot', pageSize: 8, mode: currentMode.value })
        ])
        recommendList.value = recRes.data.list || []
        hotList.value = hotRes.data.list || []
        writeCachedJSON(recommendKey, recommendList.value)
        writeCachedJSON(hotKey, hotList.value)
    } catch (error) {
        console.error('模式数据加载失败:', error)
    } finally {
        loading.value = false
    }
}

watch(currentMode, async (mode, prevMode) => {
    recommendList.value = []
    hotList.value = []
    await loadModeSections(false)
    trackBehavior('scene_mode_change', {
        sourcePage: 'home',
        extra: { mode, previousMode: prevMode }
    })
})
