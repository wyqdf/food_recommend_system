...
> 适用范围：`F:\Desktop\food-recommendation-system` 全仓  
> 最后核对日期：2026-03-08  
...
美食推荐系统，包含两套前端入口和一套后端服务：

- 用户端：菜谱浏览、搜索、推荐、收藏、评论、个人中心、投稿菜谱。
- 管理端：用户管理、食谱管理、分类管理、属性管理、统计看板。
- 后端：统一 API 服务，负责鉴权、业务逻辑、MySQL 数据访问、统计聚合、OSS 图片上传。
...
```mermaid
flowchart LR
  A[Browser] -->|http://localhost:3000| B[Vite Dev Server]
...
- `数据库修改相关操作/`
  - 数据库改动记录、SQL 片段、约束说明
- `temp/runlogs/`
  - 运行日志（目前有 `backend.log`、`frontend.log`）
...
- `admin/Dashboard.vue`：概览卡片 + 统计图表 + 高级统计
