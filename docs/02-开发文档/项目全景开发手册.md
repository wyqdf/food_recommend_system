...
> 适用范围：`F:\Desktop\大创\foodrec-merged-run` 全仓  
> 最后核对日期：2026-03-28  
...
- 后端
  ...
  - MyBatis（注解 SQL）
  - MySQL 8.0
  - Spring Data Elasticsearch（Search V2，`smartcn + combined_fields + completion`，`application.properties` 默认值为 `search.engine=elasticsearch`；本地推荐启动脚本显式使用 `SEARCH_ENGINE=auto`）
  - JWT（`jjwt 0.11.5`）
  - Lombok 1.18.38
- 前端
  - Vue 3（组合式 API）
...
- 资源服务
  - 阿里云 OSS（图片上传）
  - Elasticsearch（Search V2 搜索索引；默认配置值为 `elasticsearch`，不可用或索引未就绪时会自动回退 MySQL）
...
- 搜索服务：
  - 当前仓库默认 `search.engine=elasticsearch`
...
- `views/`
  - 首页、菜谱列表、详情、搜索、推荐、登录、注册、投稿、烹饪模式
- `views/user/`
  - 个人中心、收藏、7 日报告
- `views/admin/`
  - 管理登录、仪表盘、用户/菜谱/分类/属性/日志管理
- `components/`
  - 菜谱卡片、场景模式顶栏、冷启动问卷弹窗、复用搜索组件等
- `api/`
  - 用户端和管理端接口封装，含分类推荐、场景、埋点、烹饪会话等能力
- `stores/`
  - `user.js`、`recipe.js`、`sceneMode.js`
- `utils/`
  - 请求器、行为埋点追踪器、搜索历史工具、`latestRequest` 请求保护工具
...
2. Windows 本地推荐可直接使用根目录脚本：
   - `run-latest-dev.ps1`
   - 会停止本仓库旧进程与旧容器，并以源码方式拉起前后端
   - 后端会显式使用 `ALIYUN_OSS_ENABLED=false` 与 `SEARCH_ENGINE=auto`
   - 首次执行前仍需先完成数据库导入，并在 `frontend/` 下至少执行一次 `npm install`
3. 手动启动后端：
   - 目录：`backend/let-me-cook`
   - 命令：`mvn spring-boot:run`
   - 说明：本地默认 `aliyun.oss.enabled=false`，无需额外 OSS 环境变量即可启动
4. 如需验证 Elasticsearch 搜索：
   - 先启动 ES（例如使用根目录 `docker-compose.yml` 中的 `elasticsearch` 服务，会构建带 `analysis-smartcn` 插件的镜像）
   - 默认保持 `SEARCH_ENGINE=elasticsearch` 即可；应用会优先使用 ES，若不可读则自动回退 MySQL
   - 若首次启动时 alias 缺失但 `recipes_search_v2` 已存在且有数据，启动期会自动补回 `recipes_search -> recipes_search_v2`
5. 手动启动前端：
   - 目录：`frontend`
...
- 后端拦截器
  ...
  - `GET /api/comments/recipe/**` 允许匿名访问；若请求带有效用户 token，后端仍会注入 `userId`，用于 `isLiked` 等登录态字段
...
- 支持场景标签补全与可解释推荐理由输出。
- `SceneModeResolver` 统一处理前端 `mode` 到后端 `scene` 的映射：
  - `family -> family`
  - `fitness -> diet`
  - `quick -> quick`
  - `party -> banquet`
- `scene` 参数仍保留兼容，但首页、推荐页、菜谱列表页当前主入口已统一走 `mode`。
- `type=personal` 在“登录用户 + 命中场景 + 存在离线 Top100”时，已经改成双路独立筛选：
  - `ceil(limit * 0.7)` 目标配额优先从命中当前场景的离线 `Top100` 中按离线顺序选出
  - 若场景命中的 `Top100` 不足，不补非场景 `Top100`，而是由命中当前场景的实时结果补齐
  - 两路完成筛选后再按固定比例交错返回，不再做离线与实时混池统一排序
- 实时补位不再只从单一热门列表取数，而是同时从热门源与非热门源候选池中随机抽样，避免剩余 `30%` 全是大热门。
- `Top100` 卡片优先保留离线模型理由，仅在离线理由为空时补场景文案。
- 首页 `为你推荐` 与推荐页 `猜你喜欢` 已统一使用同一条 `type=personal` 推荐链路。
- `GET /api/categories/recommend` 不再简单等同于“按数量取前 N 个分类”，而是先按 `categories.recipe_count` 排序，再做一层多样化挑选，避免 `午餐/晚餐/热菜` 这类高重叠分类挤满入口。
...
- 收藏数字段映射已统一改为 `favorite_count`，展示口径为“导入收藏基数 + 用户收藏增量”。
...
- 同时更新 `recipes.favorite_count`。
- 启动迁移不再按 `interactions` 全量回写 `recipes.favorite_count`，避免覆盖爬虫导入的收藏基数。
...
- `GET /api/comments/recipe/**` 属于可选登录态接口：匿名请求可直接读取评论；带有效 token 时会额外识别 `userId`，用于评论点赞态等个性化字段。
...
- `recipeApi`
  - 列表、详情、搜索、建议词、推荐分类、推荐、相似菜谱、投稿
- `userApi`
  - 登录、注册、用户资料、冷启动问卷、7 日报告
- `favoriteApi`
- `commentApi`
...
- `src/utils/latestRequest.js`
  - 提供轻量级“只接受最后一次请求结果”的 guard
  - 当前 `Home.vue`、`Recommend.vue`、`Recipes.vue` 都已接入，避免切模式、切标签或切筛选时旧响应覆盖新页面状态

## 5.5 主要页面职责

### 5.5.1 用户页面

- `Home.vue`：首页 Hero 搜索、热门分类、为你推荐、热门菜谱；其中“为你推荐”走 `type=personal`，热门菜谱从更大的热门候选池中随机抽样
- `Recipes.vue`：分类/难度/排序筛选 + 分页列表，并响应全局场景模式；已使用 `latestRequest` 避免旧请求覆盖
- `RecipeDetail.vue`：详情、收藏、评论、相关推荐
- `CookingMode.vue`：步骤推进、自动保存、断点续做、完成收尾
- `Search.vue`：搜索工作台，支持建议词、最近搜索、排序、错误态与空结果态
- `Recommend.vue`：`personal / hot / new` 三标签；个人推荐受全局 `mode` 影响，热门标签下支持动态分类筛选；已使用 `latestRequest`
- `Login.vue`：用户登录页，当前只展示 5 个测试用户名用于演示，不展示偏好画像，也不暗示自动登录
...
- `SceneModeTopNav.vue`：全站场景模式切换入口
...
- `stores/recipe.js`
  - 分类缓存、当前菜谱详情
- `stores/sceneMode.js`
  - 维护全局 `family / fitness / quick / party` 模式
  - 负责主题变量切换与本地持久化
...
1. 历史库收藏基数恢复仍需确认导入口径
   - 当前已补充 `V11__backfill_recipe_favorite_count_from_rating_count.sql`，当历史导入收藏值实际落在 `rating_count` 时，可按“`rating_count` 基数 + `interactions.favorite` 增量”保守回填到 `recipes.favorite_count`。
   - 若某份历史库既没有原始导入源，也没有可复用的 `rating_count` 历史值，仍需结合备份或原始爬虫数据恢复。
...
3. ✅ `UserAuthInterceptor` 与用户侧控制器已统一使用用户 token 校验；其中 `GET /api/comments/recipe/**` 保持匿名可读、登录态增强。
