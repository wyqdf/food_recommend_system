...
> 最后核对日期：2026-03-19  
> 当前事实来源优先级：**源码 > 本文档 > `docs/` 专题文档 > 根目录旧 `readme.md`**  
> 特别提醒：根目录旧 [readme.md](./readme.md) 中关于 “Spring Boot 4”“独立 admin 前端工程” 等表述已经过时，不能再作为项目现状依据
...
4. 最后再回头看根目录旧 `readme.md`，只把它当作历史材料

---

## 附：一句话总结这个项目

这是一个**单前端工程 + 单后端工程 + MySQL + Elasticsearch** 的美食推荐系统；当前搜索已切到 **Search V2 / ES**，推荐页顶部已切到**现有分类筛选**，数据库真实结构需要同时看 **`schema.sql` + `DatabaseMigrationConfig`**，接手时最容易踩的坑是**旧 README、旧前端容器和数据库运行期补丁**。
